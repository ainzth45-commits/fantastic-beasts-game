...
describe("beastSprite — เลือกภาพตามขั้น+ท่า พร้อม fallback", () => {
  ...
  it("ร่างที่ยังไม่มีภาพ ถอยลงหาร่างที่มี (จอไม่พังระหว่างศิลป์ทยอยมา)", () => {
    // teen/grown/adult ยังไม่มี → ถอยลงมาจน junior (มีครบ)
    expect(beastSprite("teen", "happy", false)).toContain("junior-happy");
    expect(beastSprite("grown", "calm", false)).toContain("junior-idle");
    expect(beastSprite("adult", "sad", false)).toContain("junior-sad");
  });
